export const GET_ALL_USERS_START = 'GET_ALL_USERS_START';
export const GET_ALL_USERS_SUCCESS = 'GET_ALL_USERS_SUCCESS';
export const GET_ALL_USERS_FAILED = 'GET_ALL_USERS_FAILED';

export const GET_NEW_USERS_START = 'GET_NEW_USERS_START';
export const GET_NEW_USERS_SUCCESS = 'GET_NEW_USERS_SUCCESS';
export const GET_NEW_USERS_FAILED = 'GET_NEW_USERS_FAILED';

export const UPDATE_USER_START = 'UPDATE_USER_START';
export const UPDATE_USER_SUCCESS = 'UPDATE_USER_SUCCESS';
export const UPDATE_USER_FAILED = 'UPDATE_USER_FAILED';

export const DELETE_USER_START = 'DELETE_USER_START';
export const DELETE_USER_SUCCESS = 'DELETE_USER_SUCCESS';
export const DELETE_USER_FAILED = 'DELETE_USER_FAILED';

export const DELETE_MANY_USERS_START = 'DELETE_MANY_USERS_START';
export const DELETE_MANY_USERS_SUCCESS = 'DELETE_MANY_USERS_SUCCESS';
export const DELETE_MANY_USERS_FAILED = 'DELETE_MANY_USERS_FAILED';

export const DELETE_ALL_USERS_START = 'DELETE_ALL_USERS_START';
export const DELETE_ALL_USERS_SUCCESS = 'DELETE_ALL_USERS_SUCCESS';
export const DELETE_ALL_USERS_FAILED = 'DELETE_ALL_USERS_FAILED';
